...
def create_test_set(
    ...
    for idx, row in selected_df.iterrows():
        ...
        files_to_copy = [
            ...
            ('bev_occupancy_path', f"bev_occupancy_{run_id}_w{original_window_id:03d}.png"),
            ('bev_height_path', f"bev_height_{run_id}_w{original_window_id:03d}.png"),
            ('bev_density_path', f"bev_density_{run_id}_w{original_window_id:03d}.png"),
            ('bev_roughness_path', f"bev_roughness_{run_id}_w{original_window_id:03d}.png"),
        ]
        
        # Copy each file with new sequential ID
        for col_name, original_filename in files_to_copy:
            src = collection_path / original_filename
            if src.exists():
                # New filename with sequential window ID
                new_filename = original_filename.replace(f"_w{original_window_id:03d}", f"_w{idx:03d}")
...
def main():
    ...
    for i, collection in enumerate(collections):
        # Create test name
        test_name = f"real_{i+1:02d}_{collection.name[-6:]}"  # real_01_173442, etc.
        ...
        # Create test set
        metadata = create_test_set(collection, output_path, window_indices, test_name)
