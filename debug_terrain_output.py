#!/usr/bin/env python3
"""
Debug terrain flakiness - check what's being output
"""
...
def get_window_image(window_id: str, image_type: str, scenario_path: str) -> dict:
    try:
        ...
        if image_type not in valid_types:
            return {"status": "error", "error_message": f"Invalid image_type: {image_type}"}
        ...
        return {"status": "success", "image_base64": image_base64, "mime_type": "image/png"}
...
async def debug_terrain():
    """Debug what terrain is actually outputting."""
    print("\n" + "=" * 80)
    print("DEBUG: TERRAIN ANALYZER OUTPUT")
    print("=" * 80)
        ...
        if hasattr(event, 'content') and event.content and hasattr(event.content, 'parts'):
            for j, part in enumerate(event.content.parts):
                if hasattr(part, 'text'):
                    text = part.text
                    if text:
                        print(f"  Part {j} (text): {len(text)} chars")
                        print(f"    Start: {text[:100]}...")
                    else:
                        print(f"  Part {j} (text): None/Empty")


if __name__ == "__main__":
    asyncio.run(debug_terrain())
