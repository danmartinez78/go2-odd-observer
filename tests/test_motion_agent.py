...
This script is for MANUAL TESTING and inspection of the MotionAgent.
...
Usage:
    ...
    python tests/test_motion_agent.py --scenario data/test/sim_2win
    python tests/test_motion_agent.py --model gemini-2.0-flash-lite
    
Expected: JSON output with motion statistics from IMU data.
"""

from odd_agents.agents import create_motion_agent
...
def _extract_result(events: List[Any], agent_name: str = "MotionAgent") -> Optional[Dict[str, Any]]:
    """Extract final result from motion agent."""
    for event in events:
...
async def test_motion_agent(
    ...
    """Run motion agent test with specified parameters."""
    ...
    print("\n" + "=" * 80)
    print("MOTION AGENT TEST (IMU Sensor Analysis)")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Motion Agent")
    parser.add_argument(
        "--scenario",
        type=str,
